...
import { getQuizDetails } from "../../slices/createQuizSlice";
import { optionTypes } from "../../utils/helpers";
import { useCreateQuiz } from "../../queries/useCreateQuiz";
...
function QuestionsForm() {
  ...
  const { type, title } = useSelector(getQuizDetails);
  ...
  useEffect(() => {
    ...
    if (type === "") navigate("/dashboard/create-quiz");
  }, [type, navigate]);
  ...
  const handleContinue = async () => {
    ...
    createQuiz(data, {
      onSuccess: (data) => {
        navigate(
          `/dashboard/create-quiz/finish/${data?.data?.newQuiz?.shareableLink}`,
          { replace: true }
        );
